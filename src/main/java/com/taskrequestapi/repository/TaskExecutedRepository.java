...
public interface TaskExecutedRepository extends JpaRepository<TaskExecuted, Integer> {
	@Query("SELECT taskExecuted FROM TaskExecuted taskExecuted where taskExecuted.user.id = :userId AND taskExecuted.task.id =:taskId AND taskExecuted.date>=:dateBeginning and taskExecuted.date<=:dateEnd ")
